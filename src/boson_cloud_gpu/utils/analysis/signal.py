...
def dispatch_aritm_operation_kernel(kernel, nrows: int, ncols: int, *args):
...
class Signal:
    ...
    def df_dot(self):
        ...
        _df_dot = dispatch_aritm_operation_kernel(
            df_dot_kernel,
            self.nrows,
            self.ncols,
    ...
    def alpha(self):
        ...
        _alpha = dispatch_aritm_operation_kernel(
            alpha_kernel,
            self.nrows,
            self.ncols,
    ...
    def f_dot(self):
        ...
        _f_dot = dispatch_aritm_operation_kernel(
            f_dot_kernel,
            self.nrows,
            self.ncols,
    ...
    def tau_inst(self):
        ...
        _tau_inst = dispatch_aritm_operation_kernel(
            tau_inst_kernel,
            self.nrows,
            self.ncols,
            self.BH_mass.astype(PRECISION),
    ...
    def tau_gw(self):
        ...
        _tau_gw = dispatch_aritm_operation_kernel(
            tau_gw_kernel,
            self.nrows,
            self.ncols,
            self.BH_mass.astype(PRECISION),
    ...
    def undetectable_values_mask(self):
        ...
        mask = dispatch_aritm_operation_kernel(
            mask_kernel,
            self.nrows,
            self.ncols,
            self.frequency_at_detector.astype(PRECISION),
    ...
    def dispatch_mask_kernel(self):
        mask_kernel = cupy.RawKernel(
            ...
            void mask_array(
                ...
                const int nrows,
                const int ncols,
                float* masked_frequency,
                float* masked_amplitude
                )
            {
                ...
                if ((x_abs < ncols) && (y_abs < nrows))
                {
                    ...
                    {
                        masked_frequency[x_abs + ncols * y_abs] = 0;
                        masked_amplitude[x_abs + ncols * y_abs] = 0;
                    }
                }
            }
            """,
